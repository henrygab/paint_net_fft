...
namespace ArgusPaintNet.FFT.FFTWInterop
{
	...
	static class FFTW
	{
		...
		const string WisdomFile = "fftw.wisdom";
		...
		[DllImport(DllName)]
		static extern void fftw_plan_with_nthreads(int numberOfThreads);
		...
		[DllImport(DllName)]
		static extern IntPtr fftw_malloc(int size);

		[DllImport(DllName)]
		static extern void fftw_free(IntPtr ptr);

		public static void fftw_free(ref IntPtr ptr) { lock (_lock) { fftw_free(ptr); ptr = IntPtr.Zero; } }

		[DllImport(DllName)]
		static extern IntPtr fftw_alloc_real(int size);

		[DllImport(DllName)]
		static extern IntPtr fftw_alloc_complex(int size);

		[DllImport(DllName)]
		static extern void fftw_destroy_plan(IntPtr plan);
		...
		[DllImport(DllName)]
		static extern IntPtr fftw_plan_dft_r2c_2d(int n0, int n1, IntPtr doubleIn, IntPtr complexOut, PlanningFlags flags = PlanningFlags.Default);
		...
		[DllImport(DllName)]
		static extern IntPtr fftw_plan_dft_2d(int n0, int n1, IntPtr complexIn, IntPtr complexOut, int sign, PlanningFlags flags = PlanningFlags.Default);
		...
		[DllImport(DllName)]
		static extern int fftw_export_wisdom_to_filename(string filename);

		[DllImport(DllName)]
		static extern int fftw_import_wisdom_from_filename(string filename);

		static bool fftw_export_wisdom()
		{
			lock (_lock)
			{
